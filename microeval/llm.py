...
class SimpleLLMClient(ABC):
    ...
    def get_token_cost(
        ...
        pricing = pydash.get(load_models_config(), f"pricing.{self.service}", {})
                    ...
                    new_tool_calls.append(
                        {
                            "id": pydash.get(tc, "id", ""),
                            "function": {
                                "name": pydash.get(tc, "function.name"),
                                "arguments": args,
                            },
                        }
                    )
    ...
    async def get_completion(
        ...
        try:
            ...
            return self._build_success_response(
                response_text, usage, done_reason, tool_calls
            )
        except Exception as e:
    ...
    async def get_completion(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(
                    f"Unexpected error calling OpenAI: {type(e).__name__}: {e}"
                )
                raise
    ...
    async def get_embedding(self, input: str) -> List[float]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.error(
                    f"Non-retryable OpenAI error for embeddings: {type(e).__name__}: {e}"
                )
                raise RuntimeError(f"Error generating embedding: {str(e)}")
            except Exception as e:
                logger.error(
                    f"Unexpected error calling OpenAI embed: {type(e).__name__}: {e}"
                )
                raise RuntimeError(f"Error generating embedding: {str(e)}")
...
def get_aws_config():
    ...
    # Check SSO cache for expiry (more reliable than relying on frozen credentials).
    # Detect expiry outside the parse try/except so the raise isn't silently swallowed.
    if profile_name:
        sso_expired_msg = None
            ...
            formatted_tools = [
                {
                    "toolSpec": {
                        ...
                        "inputSchema": {
                            "json": pydash.get(tool, "function.parameters", {})
                        },
                    }
                }
                for tool in tools
...
def get_llm_client(client_type: LLMService, **kwargs) -> SimpleLLMClient:
    ...
    if "model" not in kwargs:
        default_models = pydash.get(
            load_models_config(), f"chat_models.{client_type}", []
        )
        if default_models:
            kwargs["model"] = (
                default_models[0]
